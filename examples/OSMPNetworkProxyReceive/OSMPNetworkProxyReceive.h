...
#define FMI_BOOLEAN_RECEIVE_IDX 5
#define FMI_BOOLEAN_LAST_IDX FMI_BOOLEAN_RECEIVE_IDX
...
/* FMU Class */
class COSMPNetworkProxyReceive {
public:
    /* FMI2 Interface mapped to C++ */
    COSMPNetworkProxyReceive(fmi2String theinstanceName, fmi2Type thefmuType, fmi2String thefmuGUID, fmi2String thefmuResourceLocation, const fmi2CallbackFunctions* thefunctions, fmi2Boolean thevisible, fmi2Boolean theloggingOn);
    ~COSMPNetworkProxyReceive();
...
protected:
...
#endif
            private_log_file << "OSMPNetworkProxy" << "::Global:FMI: " << buffer << endl;
    ...
    void internal_log(const char* category, const char* format, va_list arg)
    {
        ...
        if (!private_log_file.is_open())
            private_log_file.open(PRIVATE_LOG_PATH, ios::out | ios::app);
        if (private_log_file.is_open()) {
            private_log_file << "OSMPNetworkProxy" << "::" << instanceName << "<" << ((void*)this) << ">:" << category << ": " << buffer << endl;
            private_log_file.flush();
        }
#endif
...
protected:
    ...
    fmi2Boolean fmi_receive() { return boolean_vars[FMI_BOOLEAN_RECEIVE_IDX]; }
    void set_fmi_receive(fmi2Boolean value) { boolean_vars[FMI_BOOLEAN_RECEIVE_IDX]=value; }
